...
from .basic_modules import BasicBlock,Bottleneck,HighResolutionModule, copy_state_dict
...
class HigherResolutionNet(nn.Module):
    ...
    def load_pretrain_params(self):
        if os.path.exists('./models/pretrain_hrnet.pkl'):
            success_layer = copy_state_dict(self.state_dict(), torch.load('./models/pretrain_hrnet.pkl', map_location=self.device), prefix = '', fix_loaded=True)
            print('./models/pretrain_hrnet.pkl', "LOADING")
        # print('./models/pretrain_hrnet.pkl', "FAIL")
