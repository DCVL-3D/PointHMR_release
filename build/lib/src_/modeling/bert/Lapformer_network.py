...
class Lapformer_Body_Network(torch.nn.Module):
    ...
    def build_head(self, args):
        ...
        self.backbone = get_cls_net_gridfeat(hrnet_config, img_size=[56, 28, 14, 7], pretrained=hrnet_checkpoint)
        self.inplanes = 64  # self.backbone.backbone_channels
    ...
    def forward(self, images):
        ...
        heatmap = heatmap_.clone().flatten(2)
        ...
        idx = heatmap.argmax(2)
        sampled_feature = feature_map[:, :, idx][range(batch), :, range(batch)].contiguous()

        cam_feature = self.cam_layer(sampled_feature).transpose(1,2).contiguous()  # BX1XC

        input_feature = torch.cat([cam_feature, grid_feature.transpose(1,2).contiguous(), joint_token, sampled_feature.transpose(1,2).contiguous()],1) # BX456XC
        input_feature = torch.cat([input_feature, position_embedding],2) # BX456XC+P

        cam_parameter, pred_3d_joints, pred_3d_vertices_coarse, pred_3d_vertices_mid, pred_3d_vertices_fine = self.transformer(input_feature) # BX456XC+P
