...
class FastMETRO_Body_Network(nn.Module):
    ...
    def forward(self, cjv_token):
        ...
        cjv_features_1 = self.transformer_1(cjv_token.transpose(0,1).contiguous(), mask=None)
        cjv_features_1 = self.transformer_2(cjv_features_1, mask=None)

        # progressive dimensionality reduction
        reduced_cjv_features_1 = self.dim_reduce(cjv_features_1)

        # second transformer encoder-decoder
        cjv_features_2 = self.transformer_3(reduced_cjv_features_1, mask=None)
        cjv_features_2 = self.transformer_4(cjv_features_2, mask=None)
