...
from src.modeling._gcnn import GraphConvolution, GraphResBlock, laplacian_decomposition, laplacian_composition
...
class LapformerLayer(nn.Module):
    def __init__(self, config):
        super(LapformerLayer, self).__init__()
        self.attention = BertAttention(config)
        self.has_graph_conv = config.graph_conv
        self.mesh_type = config.mesh_type

        if self.has_graph_conv == True:
            self.graph_conv1 = GraphResBlock(config.hidden_size, config.hidden_size, mesh_type=self.mesh_type)
            self.graph_conv2 = GraphResBlock(config.hidden_size//2, config.hidden_size//2, mesh_type=self.mesh_type)
            self.graph_conv3 = GraphResBlock(config.hidden_size//4, config.hidden_size//4, mesh_type=self.mesh_type)
            print("Laphormer")
    ...
    def MHA_GCN(self, hidden_states, attention_mask, head_mask=None,
        ...
        if self.has_graph_conv==True:
            ...
            # vertices = self.graph_conv1(vertices)
            lap1, lap2, lap3 = laplacian_decomposition(vertices)
            lap1 = self.graph_conv1(lap1)
            lap2 = self.graph_conv2(lap2)
            lap3 = self.graph_conv3(lap3)
            vertices = laplacian_composition(lap1, lap2, lap3)
...
class LapformerEncoder(nn.Module):
    def __init__(self, config):
        super(LapformerEncoder, self).__init__()
        self.output_attentions = config.output_attentions
        self.output_hidden_states = config.output_hidden_states
        self.layer = nn.ModuleList([LapformerLayer(config) for _ in range(config.num_hidden_layers)])
...
class EncoderBlock(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.encoder = LapformerEncoder(config)
...
class Lapformer(BertPreTrainedModel):
    '''
    The archtecture of a transformer encoder block we used in Lapformer
    '''
    def __init__(self, config):
        super(Lapformer, self).__init__(config)
        self.config = config
