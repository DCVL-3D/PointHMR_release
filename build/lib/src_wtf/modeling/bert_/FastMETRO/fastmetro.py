...
class FastMETRO_Body_Network(nn.Module):
    def __init__(self, args, mesh_sampler, model_dim=(512,128), model_name="L", transformer_nhead=4):
        ...
        self.transformer_1 = TransformerEncoder(encoder_layer1, num_enc_layers, encoder_norm1)
        self.transformer_2 = TransformerEncoder(encoder_layer2, num_enc_layers, encoder_norm2)
    ...
    def forward(self, cjv_token):
        ...
        cjv_features_1 = self.transformer_1(cjv_token.transpose(0,1).contiguous(), mask=attention_mask)

        # progressive dimensionality reduction
        reduced_cjv_features_1 = self.dim_reduce(cjv_features_1)

        # second transformer encoder-decoder
        cjv_features_2 = self.transformer_2(reduced_cjv_features_1, mask=attention_mask)
