...
class Lapformer_Body_Network(torch.nn.Module):
    def __init__(self, args, mesh_sampler):
        ...
        self.transformer = FastMETRO_Body_Network(args, mesh_sampler, model_dim=(args.model_dim+args.position_dim,args.model_dim//4))

    def build_head(self, args):
        # self.backbone = HigherResolutionNet() # 128, 56, 56
    ...
    def forward(self, images):
        ...
        heatmap = F.softmax(heatmap_.flatten(2), dim=-1)  # BX431XHW
        heatmap_ = F.interpolate(heatmap_, scale_factor=2)
        heatmap_ = F.softmax(heatmap_.flatten(2), dim=-1).reshape(batch, -1, 2 * h, 2 * h)
